        ...
        <div className="max-w-3xl mx-auto">
          {messages.length === 0 && !loading && (
            <HowToUse
              id="query"
              steps={[
                <>Pick a <strong>commodity</strong> filter at the top to scope retrieval, or leave it on All.</>,
                <>Type a question (or click a suggested example below). Press <strong>Enter</strong> to send, <strong>Shift+Enter</strong> for a new line.</>,
                <>The answer is generated by Claude using the top-K retrieved chunks. Click <strong>N sources retrieved</strong> under any answer to see the source documents and similarity scores.</>,
              ]}
              tip="Best results come from specific questions (e.g. 'How does the FEOC rule affect South Korean anode processors?') rather than open-ended ones."
            />
          )}
          {/* Empty state */}
          {messages.length === 0 && !loading && (
