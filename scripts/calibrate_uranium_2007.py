#!/usr/bin/env python3
"""
Calibrate structural parameters for uranium 2007 Cigar Lake supply squeeze.

Cigar Lake mine (Cameco) flooded Oct 2006, removing ~15% of global uranium supply.
Nuclear Renaissance demand (30+ US reactor orders 2005-2007) compounded tightness.
EIA spot-contract price: $14.77 (2004) → $88.25 (2007) → $66.95 (2008).
2008 drop: GFC reduced electricity demand projections + financial investor exit from
uranium trusts/ETFs + Russian HEU downblending (Megatons to Megawatts) continued.

Calibration: differential_evolution maximising DA + Spearman rho - log_rmse penalty
Episode window: 2004-2008 (4 directional steps)
Critical constraint: model must predict DOWN in 2008 (DA requires 4/4 correct).
"""
...
SHOCKS = [
    ...
    # Cigar Lake flood: ~15% supply removal 2006-2008
    ShockConfig(type="export_restriction", start_year=2006, end_year=2008, magnitude=0.15),
    # Mine investor capex chill during flood uncertainty
    ShockConfig(type="capex_shock",        start_year=2006, end_year=2008, magnitude=0.30),
    # Spot peaked Jun 2007; inventory liquidation began H2 2007
    # Applied in 2007 so it drives P[2008] lower (pre-step recording in simulator)
    ShockConfig(type="stockpile_release",  start_year=2007, end_year=2007, magnitude=25.0),
    # GFC 2008: demand destruction + financial investor exit from uranium trusts/ETFs
    ShockConfig(type="macro_demand_shock", start_year=2008, end_year=2008,
                magnitude=-0.35, demand_destruction=-0.35),
]

TIME_CFG = TimeConfig(dt=1.0, start_year=2002, end_year=2010)
...
def _run(alpha_P, eta_D, tau_K, g, eia):
    ...
    cfg = ScenarioConfig(
        name="u07_cal", commodity="graphite", seed=42,
...
def main():
    ...
    print("=" * 62)
    print("Calibrating uranium_2007 episode (Cigar Lake supply squeeze)")
    ...
    bounds = [
        (0.10, 4.00),   # alpha_P
        (-0.50, -0.001),# eta_D  (uranium demand very inelastic)
        (2.00, 20.0),   # tau_K  (mines take 5-15yr to build)
        (0.95,  1.15),  # g
    ]
    ...
    print()
    print(f"_URANIUM_2007_PARAMS = dict(alpha_P={alpha_P:.3f}, eta_D={eta_D:.3f}, "
    ...
    cfg = ScenarioConfig(
        name="u07_final", commodity="graphite", seed=42,
        time=TIME_CFG, baseline=BASELINE,
        parameters=ParametersConfig(**kw),
        policy=PolicyConfig(), shocks=SHOCKS,
        outputs=OutputsConfig(metrics=["avg_price"]),
